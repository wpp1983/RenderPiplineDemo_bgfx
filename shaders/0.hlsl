...
// C:\Users\Administrator\Dropbox\work\RenderPipeline\bgfx\tools\bin\windows\shaderc.exe --platform windows -p cs_5_0 -O 1 -i ../../bgfx/scripts//../src/ --type compute --depends --debug -o 0 -f cs_clusterLightCull.sc --disasm
...
RWStructuredBuffer<uint> lightGridOffset : register(u[0]);
RWStructuredBuffer<uint> lightGridCount : register(u[1]);
RWStructuredBuffer<uint> globalLightIndexList : register(u[2]);
RWStructuredBuffer<AABB> cluster : register(u[3]);
RWStructuredBuffer<PointLight> pointLight : register(u[4]);
RWStructuredBuffer<uint> globalIndexCount : register(u[5]);
groupshared PointLight sharedLights[16*9*4];
uniform float4x4 viewMatrix;
uniform uint lightCount;
bool testSphereAABB(uint light, uint tile);
float sqDistPointAABB(float3 pointLightInfo, uint tile);
[numthreads(16, 9, 4)]
void main( int gl_LocalInvocationIndex : SV_GroupIndex , int3 gl_WorkGroupID : SV_GroupID ){
globalIndexCount[0] = 0;
uint threadCount = 16 * 9 * 4;
uint numBatches = (lightCount + threadCount -1) / threadCount;
uint tileIndex = gl_LocalInvocationIndex + 16 * 9 * 4 * gl_WorkGroupID.z;
...
lightGridOffset[tileIndex] = offset;
lightGridCount[tileIndex] = visibleLightCount;
}
bool testSphereAABB(uint light, uint tile){
float radius = sharedLights[light].range;
float4 center = mul(viewMatrix, sharedLights[light].position);
float squaredDistance = sqDistPointAABB(center.xyz, tile);
